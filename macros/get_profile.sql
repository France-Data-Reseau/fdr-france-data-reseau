    ...
    with column_profiles as (
      {% for column_name in column_names %}
        ...
        select 
          ...
          {% if dbt_profiler.is_numeric_dtype(data_type) %}avg({{ adapter.quote(column_name) }}){% else %}null::numeric{% endif %} as avg,
          {% if dbt_profiler.is_numeric_dtype(data_type) %}stddev_pop({{ adapter.quote(column_name) }}){% else %}null::numeric{% endif %} as std_dev_population,
          {% if dbt_profiler.is_numeric_dtype(data_type) %}stddev_samp({{ adapter.quote(column_name) }}){% else %}null::numeric{% endif %} as std_dev_sample,
