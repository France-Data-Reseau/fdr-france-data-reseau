...
with link_candidates as (
    ...
    {% if is_incremental() %}
      ...
      and {{ translated_source }}.last_changed > (select coalesce(max(max_translated.translated_last_changed), to_timestamp('1970-01-01T00:00:00', 'YYYY-MM-DD"T"HH24:MI:SS')) from {{ this }} max_translated where max_translated."com_name" is NULL)
    {% endif %}
