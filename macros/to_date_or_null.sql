...
{% macro to_date_or_null(column_name, source, fmts, source_alias=None) %}
...
{% else %}{# assuming text but maybe TODO convert ::text up to test ? #}
  {{ schema }}.to_date_or_null({{ source_alias if source_alias else source }}.{{ adapter.quote(column_name) }}, '{{ fmts[0] }}'::text)
{% endif %}
{% endmacro %}