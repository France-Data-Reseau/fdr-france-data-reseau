...
with imported as (
select * from {{ ref('fdr_src_communes_ods_parsed') }}
)
select {{ dbt_utils.star(ref('fdr_src_communes_ods_parsed'), except=['geo_point_2d']) }},
...
from {{ ref('fdr_src_communes_ods_parsed') }}